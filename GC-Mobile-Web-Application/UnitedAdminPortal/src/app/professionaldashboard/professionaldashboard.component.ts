...
@Component({
  ...
  styleUrls: ['./professionaldashboard.component.scss']
})
export class ProfessionaldashboardComponent implements OnInit {
 ...
 usercity = "";
  ...
  ngOnInit(): void {
    ...
    this.getCalender();
    this.getUserevents();
    this.usercity = localStorage.getItem("location");
    this.usercity = (localStorage.getItem("location") == 'Malmö')?this.cities[0]:(localStorage.getItem("location") == 'Gothenburg')?this.cities[1]:(localStorage.getItem("location") == 'Copenhagen')?this.cities[2]:(localStorage.getItem("location")== 'Stockholm')?this.cities[3]:(localStorage.getItem("location") == 'Västra Götalands län')?this.cities[4]:
    (localStorage.getItem("location") == 'Oslo')?this.cities[5]:this.cities[6];
  }
  fileChange(element: any) {
    this.uploadedFiles = element.target.files;
    console.log(this.uploadedFiles);
  }
//getevent to show on edit modal
 ...
 getevent(eventid:any) {
  ...
  Createevent(){
    ...
    console.log(this.event.eventtype["name"], this.event.mainstyle["name"], this.event.related1["name"])
...
showBasicDialog() {
 ...
 getUserevents(){
  ...
  for (let i= 0;  i < this.events.length; i++){
   ...
   if (new Date (Date.parse(this.events[i].EndDate)).toLocaleDateString() <= this.currentdate){
     this.previousevents.push(this.events[i]);
     console.log(this.events[i].EndDate)
     console.log("previous events",this.previousevents)
    }
    if(new Date (Date.parse(this.events[i].EndDate)).toLocaleDateString() > this.currentdate)
    {
...
getCalender(){
  this.eventService.getcalender().subscribe((res: any) => {
  console.log("calender",res);
  this.calender =  res.data.calendar.calenderViewLink;
